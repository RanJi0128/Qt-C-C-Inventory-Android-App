...
void ShippingTaskMenu::resizeEvent(QResizeEvent *event)
{
...
void ShippingTaskMenu::objectGeometry()
{
...
void ShippingTaskMenu::menuShow()
{
    this->close();
}
...
void ShippingTaskMenu::rowSelected(const QModelIndex & )
{
...
void ShippingTaskMenu::editShow()
{
    QItemSelectionModel *select = taskMenu->selectionModel();
    int rowNum = select->selectedRows().at(0).row();
    if(rowNum<0) return;
